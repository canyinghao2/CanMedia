...
import static com.canyinghao.otherlibrary.R.*;
...
public class ProgressWheel extends View {
    ...
    private int rimColor = 0x00000000;
    ...
    private void parseAttributes(TypedArray a) {
        barWidth = (int) a.getDimension(styleable.ProgressWheel_barWidth,
                barWidth);

        rimWidth = (int) a.getDimension(styleable.ProgressWheel_rimWidth,
                rimWidth);

        spinSpeed = (int) a.getDimension(styleable.ProgressWheel_spinSpeed,
                spinSpeed);

        delayMillis = a.getInteger(styleable.ProgressWheel_delayMillis,
                delayMillis);
        if (delayMillis < 0) {
            delayMillis = 0;
        }

        barColor = a.getColor(styleable.ProgressWheel_barColor, barColor);

        barLength = (int) a.getDimension(styleable.ProgressWheel_barLength,
                barLength);

        textSize = (int) a.getDimension(styleable.ProgressWheel_textSize,
                textSize);

        textColor = (int) a.getColor(styleable.ProgressWheel_textColor,
                textColor);

        //if the text is empty , so ignore it
        if (a.hasValue(styleable.ProgressWheel_text)) {
            setText(a.getString(styleable.ProgressWheel_text));
        }

        rimColor = (int) a.getColor(styleable.ProgressWheel_rimColor,
                rimColor);

        circleColor = (int) a.getColor(styleable.ProgressWheel_circleColor,
                circleColor);

        contourColor = a.getColor(styleable.ProgressWheel_contourColor, contourColor);
        contourSize = a.getDimension(styleable.ProgressWheel_contourSize, contourSize);
